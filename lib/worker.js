...
const worker = {};
// lookup all the cheks from database
...
worker.performCheck = (originalCheckData) => {
    ...
    //mark the outcome that has no sent yet
...
// save check outcome to database and sent to nxt process
worker.processCheckOutcome = (originalCheckData, checkOutCome) => {
...
worker.alertUserToStatusChange = (newCheckData) => {
    const message = `Alert: Your check for ${newCheckData.method.toUpperCase()} ${newCheckData.protocol}://${newCheckData.url} is currnetly ${newCheckData.state}`;
...
//timer toe xevute the worker process once per minute
worker.loop = () => {
    setInterval(() => {
        worker.gatherAllchecks();
    }, 1000*5);
